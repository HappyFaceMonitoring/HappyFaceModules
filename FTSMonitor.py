...
class FTSMonitor(hf.module.ModuleBase):
  ...
  config_keys = {
    ...
    'failed_channels_critical': ('number of channels above failed_transfers_threshold that triggers a critical status', ''),
    'in_channel_filter_string': ('substring that needs to be contained in channel name to mark incoming channel', ''),
    'out_channel_filter_string': ('substring that needs to be contained in channel name to mark outgoing channel', ''),
    'source_url': ('URL to FTS Monitor', ''),
  ...
  def extractData(self):
	...
	if self.in_channel_filter_string in channel:
	  in_channel_stats = {'Ready': int(0), 'MembersFrom': int(0), 'MembersTo': int(0), 'Active': int(0), 'Finished': int(0), 'FinishedDirty': int(0), 'Failed': int(0), 'Canceled': int(0)}
	  in_channel_stats['Channel'] = channel
	  for i in range(0,len(rowlist[irow][1])):
	    strvalue = str(rowlist[irow][1][i].get('style')).replace('width: ','').replace('%','').replace('None','0')
	    in_channel_stats[str(rowlist[irow][1][i].get('class'))] += int(strvalue)
	  ...
	  out_channel_stats = {'Ready': int(0), 'MembersFrom': int(0), 'MembersTo': int(0), 'Active': int(0), 'Finished': int(0), 'FinishedDirty': int(0), 'Failed': int(0), 'Canceled': int(0)}
	  out_channel_stats['Channel'] = channel
	  for i in range(0,len(rowlist[irow][1])):
	    strvalue = str(rowlist[irow][1][i].get('style')).replace('width: ','').replace('%','').replace('None','0')
	    out_channel_stats[str(rowlist[irow][1][i].get('class'))] += int(strvalue)
	...
	try:
	  ...
	  MembersTo = 1
	# look up current channel and add member numbers on 'to' and 'from' side of channel to in_channel_stats
	...
	try:
	  ...
	  MembersTo = 1
	# look up current channel and add member numbers on 'to' and 'from' side of channel to out_channel_stats
    ...
    if iBreachIn >= self.failed_channels_critical:
      ...
      data['in_channels_above_failed_threshold_color'] = '' # 'ok' colors cell green, '' leaves it blank
    ...
    if iBreachIn >= self.failed_channels_critical or iBreachOut >= self.failed_channels_critical:
      data['status'] = 0.0
    elif iBreachIn >= self.failed_channels_warning or iBreachOut >= self.failed_channels_warning:
      data['status'] = 0.5
    else:
      data['status'] = 1.0
  ...
  def getTemplateData(self):
    ...
    out_details_list = self.subtables['out_channel_stats'].select().where(self.subtables['out_channel_stats'].c.parent_id==self.dataset['id']).execute().fetchall()
