...
class DashboardSummary(hf.module.ModuleBase):
    ...
    config_keys = {
        'source_url' : ('Dashboard Summary URL', \
            'both||http://dashb-ssb.cern.ch/dashboard/request.py/getsiteplotdata?site=T1_DE_KIT&view=all&time=24&prettyprint=true'),
    ...
    def extractData(self):
        ...
        # Save determined status <--- disabled for the timebeing
    ...
    def getTemplateData(self):
        ...
        formatted_history_data_list = self.subtables['formatted_history_data'].select().\
            where(self.subtables['formatted_history_data'].c.parent_id==self.dataset['id']).\
            execute().fetchall()
        ...
        link_url_template = ("http://dashb-ssb.cern.ch/dashboard/request.py/"
            "sitehistory?site={SITE}#currentView={VIEW}"
            "&time=24&start_date=&end_date=&values=false"
            "&spline=false&white=false")
        data['link_url'] = link_url_template.format(SITE=self.config['tier_name'], VIEW=self.config['view_option'])
        return data
