...
class PlotdCache(object):
    ...
    def prepareAcquisition(self):
        ...
        self.plotname = "testpicture.png"
        self.config = {'category': 'host',
                       'host': 'f01-120-126,f01-120-127,f01-120-128,f01-120-129,'
                                                   'f01-124-126,f01-124-127,f01-120-128,f01-120-130',
                       'pool': 'D_cms,sT_cms,wT_cms,S_cms,e_ops'}
        ...
        for entry in sorted(json.loads(response.read()), key=lambda x: x.get('moverStart')):
            for element in objects_of_interest:
                ...
                if searchresult is not None:
                    if (element in entry.get('pool')) or (element in searchresult.group()):
                        if self.config['category'] == 'pool':
                            pool = entry.get('pool')
                        else:
                            if self.config['category'] == 'host':
                                pool = searchresult.group()
    ...
    def plot(self):
        ...
        # save figure
        fig.savefig(self.plotname, dpi=91, bbox_inches="tight")
        fig_diff.savefig(self.plotname.replace(".png", "_timedifferences.png"), dpi=91, bbox_inches="tight")

if __name__ == "__main__":
    testobject = PlotdCache()
    testobject.prepareAcquisition()
    testobject.extractData()
    testobject.plot()
